...
# pour alléger le fichier final, nous allons arrondir les élévations au mètre
...
::: bloc_aller_loin-body
Les mesures appartenant à la famille des *Two Step Floating Catchement Area* permettent de mesurer l'accessibilité à des aménités tout en tenant compte de leur disponibilité compte tenu de la population vivant à proximité de ces aménités. Le calcul de ce type de mesure implique deux étapes comme leur nom le suggère. Si nous conservons ici l'exemple des parcs, la première étape consiste à calculer pour chaque parc le nombre de personne habitant à proximité du parc. 

$$
 R_{j} = \frac{S_j}{\sum^{n}_{i=1}{P_i \times W(d_{ji}) }}
$$ {#eq-TSFCARj}

Avec :
* $j$ un parc
* $S_j$ la capacité du parc $j$ (ici sa superficie)
* $i$ une entité géographique (ici une aire de diffusion)
* $d_{ij}$ la distance entre $j$ et $i$
* $P_i$ la population au sein de $i$
* $W$ une fonction de pondération permettant de contrôler le fait qu'un parc plus éloigné est moins attractif et contribue moins au score final d'accessibilité.
* $R_j$ le nombre de personnes situées à proximité du parc $j$

Nous obtenons ainsi $R_j$, soit le ratio entre la capacité du parc et le total de la population ayant accès au parc. Il mesure donc le niveau de disponibilité du parc en hectare par habitant.
...
Nous obtenons au final $A_i$, soit pour chaque aire de diffusion le nombre total d'hectare de parc disponible par habitant.

Notez que la fonction $W$ joue un rôle très important dans la formulation du 2FCA. Elle peut prendre plusieurs formes, la plus simple étant une fonction binaire donnant un poid de 1 à des parcs situés en dessous d'une certaine distance. Par exemple, la fonction R ci-dessous donne un poids de 1 à des parcs situés à moins de 500m et 0 au delà.
...
Le problème de cette formulation est qu'elle implique que l'accessibilité au parc est la même si l'on est situé à 25m du parc et 500m. Il a donc été proposé d'améliorer la méthode en ajoutant une fonction de pondération par pallier, c'est à dire accordant un poids différent selon des seuils de distances prédéfinis. Les fonctions R ci-dessous font ainsi varier le poids dans trois catégories de distance selon une pondération appelée *slow decay* et *fast decay* [@luo2009enhanced] et des palliers de 150m, 300m et 500m.
...
Finalement, il a aussi été proposé d'utiliser des fonctions de pondération continues et décroissantes. Les fonctions R ci-dessous illustrent une fonction incluant un effet de pallier tel que proposé par McGrail [-@mcgrail2012spatial] et une fonction logistique décroissante.
...
# pallier à 125m, maximum à 500m
W_cont_pallier <- function(x){
...
Concernant la fonction logistique, elle comporte deux paramètre, $\alpha$ et $\beta$. Le premier paramètre permet de contrôler la distance à laquelle la pondération atteindra la valeur de 0,5. Le second contrôle la vitesse de décroissance de la courbe.

La figure ci-dessous permet d'illustrer les poids obtenus selon chacune de ces fonctions de pondération.
...
Le choix d'une fonction de pondération doit avant tout reposer sur des considérations théoriques et représenter correctement la décroissance de l'attractivité d'une aménité en fonction de sa distance d'éloignement. Dans l'idéal, ces fonctions devraient être ajustées en fonction de données sur les habitudes de déplacement des individus.
:::
:::
...
# calcul de la distance moyenne aux trois supermarchés les plus proches
...
# Notez ici que nous pouvons avoir des origines pour lesquels
# n'avons pas 3 supermarchés à moins de 240 minutes.
# pour ces quelques cas, nous allons ajouter des temps de 240 minutes pour les 
...
# nous pouvons a présent fusionner nos différent dataframe avec nos 
...
# nous utilisons la pondération de gradiant continu du E2SFCA
Wfun <- function(x){
...
# on va calculer le résultat en milliers d'habitants
