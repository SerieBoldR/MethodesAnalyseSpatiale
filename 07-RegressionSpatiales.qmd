...
::: bloc_attention-body
La formulation d'un modèle SLX, implique deux types d'effets de la part des variables $X$ : les effets directs et indirects. En effet, si pour une obseration $i$, la valeur d'une caractéristique $X$ augmente, alors la valeur $y_i$ va changer en fonction du coefficient $\beta$. Il s'agit de l'effet direct. De plus, les valeurs des moyennes spatiales ($WX$) de voisins $j$ de $i$ vont aussi être amenées à augmenter, impactant alors les valeurs $y_j$ des voisins selon les coefficients $\theta$.

Prenons un exemple concret et admettons que nous construisions un modèle ayant pour objectif de prédire le prix des maisons sur un territoire à partir d'un ensemble de caractérisiques des maisons. Admettons que la taille du jardin associé à la maison fasse partie des variable $X$ et que nous ayons aussi intégré une version spatialement décalée de cette variable dans notre modèle. Dans ce cas précis, pour une maison $i$, admettons qu'un jardin plus grand augmente le prix de la maison $i$. Cette augmentation de la taille du jardin va aussi avoir un impact sur le prix des maisons voisines car leur propre prix est dépendant de la taille des jardins des maisons voisines. Ainsi, chaque maison $j$, voisine de $i$ verra son prix augmenter à cause de l'augmentation de la taille du jardin de la maison $i$.

Pour capturer l'impact total sur le prix des maisons de l'aumgentation de la taille du jardin dans $i$, on va donc combiner son effet direct (augmentation du prix de la maison $i$) et son effet indirect (augmentation du prix des maisons $j$) pour obtenir son effet total.
...
```
En regardat les valeurs de *p*, nous pouvons constater que dans ce modèle, seule la variable `Pct0_14` a un impact direct et indirect significatif au seuil 0.01. L'augmentation de un point de pourcentage de la population des 14 ans et moins est donc associé localement à une réduction de 0.2 de la concentration de NO2 annuelle. Chez les entités voisines, cette réduction est de 0.78 et l'effet total est donc une réduction de 0.98.
...
::: bloc_attention-body
Tout comme le modèle SLX vu ci-dessus, la formulation du modèle SAR-LAG implique des effets particuliers. En effet, reprenons l'exemple d'un modèle prédisant le prix de vente des maisons et admettons que nous utilisons cette fois-ci un modèle de type SAR-LAG.

1. L'augmentation de la taille du jardin de la maison $i$ va augmenter le prix de la maison $i$ ($y_i$).
2. Cette augmentation de prix de la maison $i$ va avoir un impact sur les voisins de $i$, soit les maisons $j$ car le prix de ces maisons dépend du prix de la maison $i$ au travers du terme $Wy\rho$ du modèle. Si $\rho$ vaut par exemple 0.8, alors 80% de l'augmentation du prix de $i$ va se répercuter sur le prix des maisons $j$.
3. De même, les voisines des maisons $j$, les maisons $k$ vont aussi être impactées par le changement de prix des maisons $j$ et ainsi de suite de voisin en voisins.
4. Au final, la maison $i$ verra son propre prix augmenter encore plus car le prix de ces voisines aura augmenté par effet de rétro actions.

Ce processus de propagation est appelé l'effet de déversement (*spillover*) en économétrie. 

L'effet original de l'augmentation de la taille du jardin sur la maison $i$, combiné à l'augmentation par rétro-action est appelé l'effet direct.

L'effet cumulé de l'augmentation de la taille du jardin sur toutes les autres maisons ($\neq i$) est appelé l'effet indirect.

La somme des effets indirects et des directs est appelé l'effet total.
...
```
L'interprétation des effets directs se rapproche de celle des coefficients classiques. Ainsi, selon ce modèle, l'augmentation du niveau de vie médian de 1000€ dans un IRIS serait associée avec une diminution moyenne de la concentration de NO2 de 0.14 dans cet IRIR. L'effet total est de -0.82, indiquant qu'en moyenne, l'augmentation de 1000€ du niveau de vie médian dans un IRIS serait associé avec une diminution moyenne de 0.82 de la concentration de NO2 dans l'ensmble des IRIS. Au final, l'effet indirect est simplement la différence entre l'effet total et l'effet direct. Nous pouvons constater ici que les effets indirects sont plus importants que les effets directs.
...
Dans le modèle SEM (aussi appelé *SAR-ERROR*), l'intégration de l'autocorrélation spatiale est réalisée sur le terme d'erreur, ce qui pourrait se justifier par l'omission d'une variable dépendante spatialement structurée [@dube2014econometrie, p. 126]. L'équation du modèle SEM s'écrit :
...
::: bloc_attention-body
Pour une description détaillée des modèles généralisés additifs, nous vous invitons vivement à lire le [chapitre suivant](https://laeq.github.io/LivreMethoQuantBolR/chap11.html) [@RBoldAir].
:::
:::
...
Correction à la [section @sec-10073].
:::
:::
